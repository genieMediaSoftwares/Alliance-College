...
// const shareUrl = `https://www.alliancemgt.org/og.php?slug=${blog.permalink}`;
...
/* ═══════════════════════════════════════════════════════
   BLOG CARD
═══════════════════════════════════════════════════════ */
const FALLBACK_IMG = "https://images.unsplash.com/photo-1499750310107-5fef28a66643?w=800&q=80";
    ...
    >
      {tags.map((tag, idx) => (
        <span key={idx} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold"
          style={{ background: "#F3EBE3", color: "#6B4A2D", border: "1px solid #D4B49A" }}>
          {tag}
          <button type="button" onClick={(e) => { e.stopPropagation(); removeTag(idx); }}
            className="ml-0.5 rounded-full hover:bg-[#6B4A2D]/20 p-0.5 transition" style={{ color: "#6B4A2D" }}>
            <X size={10} strokeWidth={2.5} />
          </button>
        </span>
      ))}
...
═══════════════════════════════════════════════════════ */
const CATEGORIES = [
  "Services", "Corporate Shoots", "Event Shoots", "Podcast Shoots",
  "Professional Shoots", "Business Portfolio", "UI/UX Design",
  "Animation 2d or 3d", "Video Editing", "Podcast", "Figma", "Technology", "News",
];
...
export default function AdminBlogs() {
  ...
  /* ══════════════════════════════════════════════
     FETCH BLOGS
     Returns the fetched array so callers can use
     it immediately without waiting for setState.
  ══════════════════════════════════════════════ */
  const fetchBlogs = useCallback(async () => {
    ...
    try {
      ...
      const sorted = Array.isArray(data)
        ...
        : [];
      setBlogs(sorted);
      return sorted; // ← caller can use this immediately
    } catch (err) {
  ...
  /* ══════════════════════════════════════════════
     HANDLE SUBMIT
     ─────────────────────────────────────────────
     FIX: The issue was that setActiveTab was being
     called before the blog list was updated in state.
     This caused the tab to switch to a list that
     hadn't been refreshed yet.

     SOLUTION: Await fetchBlogs() completely, then
     setActiveTab. This ensures the blog list in
     state already contains the newly created/updated
     blog when the tab switches.
  ══════════════════════════════════════════════ */
  const handleSubmit = async (submitAsDraft) => {
    // Capture as a local const immediately — immune to any re-renders
    ...
    try {
      ...
      if (result.success) {
        ...
        // ★ CRITICAL FIX: Await fetchBlogs so blogs state is populated BEFORE we switch tabs.
        //   Without this await, setActiveTab fires before setBlogs, so the grid
        //   appears empty for a render cycle — making it look like nothing happened.
        await fetchBlogs();

        // ★ Now switch to the correct tab — blogs are already loaded
        //   If editing an existing blog, we don't automatically switch tabs,
        //   but if it's a new blog, we switch to the appropriate tab.
        if (!editingId) {
          setActiveTab(isDraft ? "drafts" : "published");
        } else {
          // When editing, stay on the create tab, but the user might want to see the result
          // Optionally, you could navigate to the list view, but staying is fine.
          // However, to match the expected behavior, we'll switch to the appropriate list.
          setActiveTab(isDraft ? "drafts" : "published");
        }
  ...
  /* ══════════════════════════════════════════════
     PUBLISH A DRAFT (from card → modal)
     Same fix: await fetchBlogs() before setActiveTab
  ══════════════════════════════════════════════ */
  ...
  const handleUnpublish = async (blog) => {
    try {
      ...
      if (result.success) {
        showToast("Blog moved to drafts — hidden from users.");
        // ★ Await fetch, then switch to drafts tab so user sees it moved
        await fetchBlogs();
        setActiveTab("drafts");
      } else {
  ...
  return (
    <main className="w-full min-h-screen bg-[#F7F6F3] overflow-x-hidden font-sans">
      <style>{`
        ...
        .editor-wrap:focus-within{border-color:#6B4A2D;box-shadow:0 0 0 4px rgba(107,74,45,.08)}
      `}</style>
            ...
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search by title or description…"
                  ...
                  className="w-full pl-9 pr-4 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-[#6B4A2D] outline-none transition" />
              </div>
            </div>
            <div className="w-full sm:w-52">
        ...
        <section className="max-w-3xl mx-auto px-3 sm:px-6 py-6 sm:py-10" ref={formTopRef}>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div className="px-5 sm:px-8 py-5 border-b border-gray-100 flex items-center gap-4">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ background: "#6B4A2D" }}>
                {editingId ? <Edit2 size={18} color="#fff" /> : <FileText size={18} color="#fff" />}
              </div>
              <div>
            ...
            {/* onSubmit is a no-op — buttons call handleSubmit() directly */}
            <form onSubmit={(e) => e.preventDefault()} className="px-4 sm:px-8 py-6 sm:py-8 space-y-5 sm:space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-gray-100">
                ...
                <button
                  ...
                  className="flex-1 py-3 sm:py-3.5 rounded-xl font-extrabold text-sm text-white transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
                  style={{ background: submitting ? "#9d7a5f" : "#6B4A2D" }}
                >
