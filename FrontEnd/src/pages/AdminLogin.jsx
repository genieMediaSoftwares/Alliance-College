      ...
      <section className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 px-3 sm:px-4 md:px-6 py-6 sm:py-8 md:py-12 lg:py-16">
        <div className="w-full max-w-sm md:max-w-md lg:max-w-md">
            ...
            <form onSubmit={handleLogin} className="space-y-4 md:space-y-5" noValidate>
              <div>
                ...
                <div className="relative">
                  <input id="email" type="email" placeholder="admin@alliancemgt.org" value={email} onChange={(e) => { setEmail(e.target.value); if (error) setError(""); }} autoComplete="email" required className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-gray-200 rounded-lg bg-gray-50 text-gray-900 placeholder-gray-400 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-200 focus:bg-white outline-none transition duration-200 hover:border-gray-300 text-sm sm:text-base"/>
              ...
              <div>
                ...
                <div className="relative">
                  <input id="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" value={password} onChange={(e) => { setPassword(e.target.value); if (error) setError(""); }} autoComplete="current-password" required className="w-full px-3 sm:px-4 py-2.5 sm:py-3 pr-10 sm:pr-12 border-2 border-gray-200 rounded-lg bg-gray-50 text-gray-900 placeholder-gray-400 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-200 focus:bg-white outline-none transition duration-200 hover:border-gray-300 text-sm sm:text-base"/>
                  <button type="button" onClick={() => setShowPassword((prev) => !prev)} aria-label={showPassword ? "Hide password" : "Show password"} className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-yellow-600 transition duration-200 p-1" >
              ...
              <div className="flex items-center text-xs sm:text-sm">
                <label className="flex items-center gap-2 cursor-pointer group select-none">
                  <input type="checkbox" className="w-4 h-4 rounded border-2 border-gray-300 cursor-pointer accent-yellow-500 transition" />
              ...
              {error && (
                <div role="alert" className="bg-red-50 border-2 border-red-300 text-red-700 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg text-xs sm:text-sm font-medium">
                  ⚠️ {error}
                </div>
              )}
          ...
          <p className="text-center text-xs text-gray-600 mt-4 sm:mt-6 px-2 font-medium">
            ...
            <a href="#" className="text-yellow-600 hover:text-amber-600 font-bold transition" >
              Terms of Service
            </a>
          </p>
        </div>
